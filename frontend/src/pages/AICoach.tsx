...
export default function AICoach() {
  ...
  const sendMessage = async () => {
    ...
    await api.createChatMessage('user', userMsg.content);

    const [trades, biases] = await Promise.all([
      api.getTrades(50, 'desc'),
      api.getBiasAnalyses(),
    ]);
    ...
    try {
      ...
      const aiContent = response.reply || 'I apologize, I could not generate a response. Please try again.';
      const aiMsg: ChatMessage = { role: 'assistant', content: aiContent };
      setMessages(prev => [...prev, aiMsg]);
      await api.createChatMessage('assistant', aiContent);
    } catch {
