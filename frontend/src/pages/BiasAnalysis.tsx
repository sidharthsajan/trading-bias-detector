...
export default function BiasAnalysis() {
  ...
  const fetchData = async () => {
    const [tradesList, biasList] = await Promise.all([
      api.getTrades(500, 'asc'),
      api.getBiasAnalyses(),
    ]);
    setTrades((tradesList || []).map((t: any) => ({
  ...
  const runAnalysis = async () => {
    ...
    await api.deleteBiasAnalyses();
    if (biases.length > 0) {
      await api.createBiasAnalyses(
        biases.map(b => ({
